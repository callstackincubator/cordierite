...
- **End-to-end check** that SPKI pins in `app.json` match the host key material certs while tools run from the **CLI**, not in-app menus.
...
- **Pin mismatch**: change the host key without updating pins—expect a **clean failure** at TLS (pinning doing its job).
